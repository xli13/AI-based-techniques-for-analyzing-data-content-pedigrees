...
class MinHash(object):
    ...
    def update_batch(self, b):
        ...
        split_sage = 1000
        len_hv = int(len(hv) / split_sage)

        if len_hv > 0:
            hv_split = hv[:split_sage]
            hv_split = np.bitwise_and(((hv_split * np.tile(a, (split_sage, 1)).T).T + b) % _mersenne_prime, _max_hash)

            for i in range(1, len_hv - 1):
                hv_tmp = np.bitwise_and(
                    ((hv[i * split_sage:(i + 1) * split_sage] * np.tile(a, (split_sage, 1)).T).T + b) % _mersenne_prime,
                    _max_hash)
                hv_split = np.vstack([hv_split, hv_tmp]).min(axis=0)
            phv_res = hv[len_hv * split_sage:]
            phv_res = (((phv_res * np.tile(a, (len(phv_res), 1)).T).T) + b) % _mersenne_prime
            phv = np.bitwise_and(phv_res, _max_hash)
            # phv = np.bitwise_and(hv_sum, _max_hash)
            phv = np.vstack([hv_split, phv]).min(axis=0)
        else:
            phv = np.bitwise_and(((hv * np.tile(a, (len(hv), 1)).T).T + b) % _mersenne_prime,
                                 _max_hash)  # np.tile(a, (len(hv), 1))
